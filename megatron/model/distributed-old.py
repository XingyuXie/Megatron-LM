            ...
            if param.grad is not None:
                ...
                if self._before_opt_step and hasattr(param, 'local_error') \
                    ...
                    if not self._init_grad_flag:
                        local_ref.copy_(param.data).mul_((1.0-self.ref_beta)/eta).add_(local_main_grad, alpha = -1.0)
                        theta_2.copy_(param.data).mul_(self.ref_beta).mul_(-1.0)
                        # local_error.add_(param.data, alpha = gamma)
                    ...
                    # pre_grad.mul_(self.ref_beta).add_(local_var, alpha = -(1.-self.ref_beta))
                        ...
                        local_var.copy_(param.data)
                        # pre_grad.add_(local_main_grad, alpha = (1.0-self.ref_beta))
                        # inv_local_lr = (torch.sqrt(pre_grad).add_(1e-8)).div(self.ref_lr).mul_(param.data)
                        local_ref.add_(param.data, alpha = -(1.0-self.ref_beta)/eta).add_(local_main_grad)

                    torch.cuda.nvtx.range_push("Err-FeedBack&Ref-Point")

                    local_error.add_(param.data, alpha = -1.0)
                    param.data.mul_(1.0-gamma).add_(local_ref, alpha = -eta)

                    local_ref.add_(param.data, alpha = (1.0-self.ref_beta)/eta).add_(local_main_grad, alpha = -1.0)

                    theta_2.add_(local_var, alpha = gamma-self.ref_beta)

                    local_error.add_(theta_2)
                    local_main_grad.copy_(local_error)
                    local_error.add_(param.data, alpha = -1.0)
